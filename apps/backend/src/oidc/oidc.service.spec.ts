...
describe("OidcService", () => {
  ...
  it("discovers configuration on init", async () => {
    (client.discovery as jest.Mock).mockResolvedValueOnce(makeConfig(true));
  ...
  it("generateAuthUrl with PKCE support does not include state", async () => {
    ...
    const buildSpy = client.buildAuthorizationUrl as jest.Mock;
  ...
  it("generateAuthUrl without PKCE support includes state", async () => {
    (client.discovery as jest.Mock).mockResolvedValueOnce(makeConfig(false));
    (client.randomPKCECodeVerifier as jest.Mock).mockReturnValue(
      "verifier-xyz",
    );
    (client.calculatePKCECodeChallenge as jest.Mock).mockResolvedValue(
    ...
    (client.randomState as jest.Mock).mockReturnValue("state-789");
  ...
  it("handleCallback exchanges code using authorizationCodeGrant", async () => {
    (client.discovery as jest.Mock).mockResolvedValueOnce(makeConfig(true));
    const grantSpy = client.authorizationCodeGrant as jest.Mock;
  ...
  it("getUserInfo returns user claims", async () => {
    (client.discovery as jest.Mock).mockResolvedValueOnce(makeConfig(true));
    (client.fetchUserInfo as jest.Mock).mockResolvedValue({ sub: "123" });
  ...
  it("buildLogoutUrl includes optional post_logout_redirect_uri when provided", async () => {
    (client.discovery as jest.Mock).mockResolvedValueOnce(makeConfig(true));
    const endSessionSpy = client.buildEndSessionUrl as jest.Mock;
