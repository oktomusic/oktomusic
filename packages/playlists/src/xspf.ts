...
import type { JspfAttributionItem, JspfLinkMetaItem, JspfPlaylist, JspfTrack } from "./jspf"
...
function rawLinkMetaToJspf(
  ...
  const result: JspfLinkMetaItem[] = (arr as readonly RawLinkMetaItem[]).map((item) => ({
    [item["@_rel"]]: item["#text"] ?? "",
  }))
  return result.length > 0 ? result : undefined
}

function rawAttributionToJspf(raw: RawAttribution | undefined): readonly JspfAttributionItem[] | undefined {
...
function rawTrackToJspf(raw: RawTrack): JspfTrack {
  ...
  if (raw.trackNum != null) {
    const n = typeof raw.trackNum === "string" ? parseInt(raw.trackNum, 10) : raw.trackNum
    if (!isNaN(n)) track.trackNum = n
  }
  if (raw.duration != null) {
    const d = typeof raw.duration === "string" ? parseInt(raw.duration, 10) : raw.duration
...
export function parseXspf(input: string): JspfPlaylist {
  ...
  if (rawPlaylist.creator != null) playlistData.creator = String(rawPlaylist.creator)
  if (rawPlaylist.annotation != null) playlistData.annotation = String(rawPlaylist.annotation)
  if (rawPlaylist.info != null) playlistData.info = String(rawPlaylist.info)
  if (rawPlaylist.location != null) playlistData.location = String(rawPlaylist.location)
  if (rawPlaylist.identifier != null) playlistData.identifier = String(rawPlaylist.identifier)
  ...
  if (rawPlaylist.license != null) playlistData.license = String(rawPlaylist.license)
  ...
  if (!result.success) {
    throw new Error(`Invalid XSPF: schema validation failed: ${result.error.message}`)
  }
  return result.data
}
...
function xmlTagAttr(tag: string, attrs: Record<string, string>, content: string, indent = ""): string {
...
function buildTrackXml(track: JspfTrack): string {
  ...
  if (track.creator != null) lines.push(xmlTag("creator", track.creator, "      "))
  if (track.annotation != null) lines.push(xmlTag("annotation", track.annotation, "      "))
  ...
  if (track.trackNum != null) lines.push(xmlTag("trackNum", String(track.trackNum), "      "))
  if (track.duration != null) lines.push(xmlTag("duration", String(track.duration), "      "))
...
export function generateXspf(playlist: JspfPlaylist): string {
  ...
  if (p.annotation != null) lines.push(xmlTag("annotation", p.annotation, "  "))
  if (p.info != null) lines.push(xmlTag("info", p.info, "  "))
  if (p.location != null) lines.push(xmlTag("location", p.location, "  "))
  if (p.identifier != null) lines.push(xmlTag("identifier", p.identifier, "  "))
  ...
  if (p.attribution && p.attribution.length > 0) {
    ...
    for (const item of p.attribution) {
      if (item.location != null) lines.push(xmlTag("location", item.location, "    "))
      if (item.identifier != null) lines.push(xmlTag("identifier", item.identifier, "    "))
    }
    lines.push("  </attribution>")
  }
